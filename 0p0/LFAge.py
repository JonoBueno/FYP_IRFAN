...
for label, r, M in zip(labels, rs, Ms):
    if M>0.1:
        ...
        ls = (10**df["log_L"].values) * 3.828e33 * 6.242e8  # Convert luminosity from ergs to meV
        age = df['star_age'].values
        F = [Flux(l, r * 3.086e18) for l in ls]  # Calculate flux for each luminosity and distance
