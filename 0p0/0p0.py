...
for file in files:
    ...
    if condition.any():
        ...
        start_h_burning = (
            None  # Or assign a default value like None or -1 if no value is found
        )
...
for i, (file, label, Hb, eHb) in enumerate(zip(files, labels, startHb, endHb)):
    ...
    plt.plot(data.T[0], data.T[1], color="red", label=label)
    ...
    plt.text(
        Hb - 1e2,
        df.loc[df["star_age"] == Hb, "log_Lneu"].values[0] - 1,
        label,
        color="blue",
        fontsize=12,
        ha="center",
    ...
    if i == len(files) - 1:
        plt.text(
            Hb,
            df.loc[df["star_age"] == Hb, "log_Lneu"].values[0] + 0.1,
            "ZAMS",
            color="blue",
            fontsize=12,
            ha="center",
    ...
    if i == len(files) - 1:
        plt.text(
            eHb,
            df.loc[df["star_age"] == eHb, "log_Lneu"].values[0] + 2.1,
            "TAMS",
            color="red",
            fontsize=12,
            ha="center",
    ...
    X = df[
        ["star_age"]
    ].values  # 2D array for features (star_age as the single feature)
